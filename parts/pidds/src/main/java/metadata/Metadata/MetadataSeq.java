...
/**
* A sequence of Metadata instances.
*/
public final class MetadataSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public MetadataSeq (int initialMaximum) {
        super(Metadata.class, initialMaximum);
    }

    public MetadataSeq (Collection elements) {
        super(Metadata.class, elements);
    }

    public  Metadata get(int index) { 
        return (Metadata) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, Metadata.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' Metadata objects (beyond the original size of this object)
