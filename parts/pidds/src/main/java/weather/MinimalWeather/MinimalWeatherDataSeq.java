...
/**
* A sequence of MinimalWeatherData instances.
*/
public final class MinimalWeatherDataSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public MinimalWeatherDataSeq() {
        super(MinimalWeatherData.class);
    }

    public MinimalWeatherDataSeq (int initialMaximum) {
        super(MinimalWeatherData.class, initialMaximum);
    }

    public MinimalWeatherDataSeq (Collection elements) {
        super(MinimalWeatherData.class, elements);
    }

    public  MinimalWeatherData get(int index) { 
        return (MinimalWeatherData) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, MinimalWeatherData.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' MinimalWeatherData objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(MinimalWeatherData.create());
