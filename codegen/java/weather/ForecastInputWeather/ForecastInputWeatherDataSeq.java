...
/**
* A sequence of ForecastInputWeatherData instances.
*/
public final class ForecastInputWeatherDataSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public ForecastInputWeatherDataSeq() {
        super(ForecastInputWeatherData.class);
    }

    public ForecastInputWeatherDataSeq (int initialMaximum) {
        super(ForecastInputWeatherData.class, initialMaximum);
    }

    public ForecastInputWeatherDataSeq (Collection elements) {
        super(ForecastInputWeatherData.class, elements);
    }

    public  ForecastInputWeatherData get(int index) { 
        return (ForecastInputWeatherData) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, ForecastInputWeatherData.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' ForecastInputWeatherData objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(ForecastInputWeatherData.create());
