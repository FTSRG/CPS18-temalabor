...
/**
* A sequence of AirportWeatherData instances.
*/
public final class AirportWeatherDataSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public AirportWeatherDataSeq() {
        super(AirportWeatherData.class);
    }

    public AirportWeatherDataSeq (int initialMaximum) {
        super(AirportWeatherData.class, initialMaximum);
    }

    public AirportWeatherDataSeq (Collection elements) {
        super(AirportWeatherData.class, elements);
    }

    public  AirportWeatherData get(int index) { 
        return (AirportWeatherData) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, AirportWeatherData.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' AirportWeatherData objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(AirportWeatherData.create());
