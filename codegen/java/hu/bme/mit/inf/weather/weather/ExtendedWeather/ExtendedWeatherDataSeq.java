...
/**
* A sequence of ExtendedWeatherData instances.
*/
public final class ExtendedWeatherDataSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public ExtendedWeatherDataSeq() {
        super(ExtendedWeatherData.class);
    }

    public ExtendedWeatherDataSeq (int initialMaximum) {
        super(ExtendedWeatherData.class, initialMaximum);
    }

    public ExtendedWeatherDataSeq (Collection elements) {
        super(ExtendedWeatherData.class, elements);
    }

    public  ExtendedWeatherData get(int index) { 
        return (ExtendedWeatherData) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, ExtendedWeatherData.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' ExtendedWeatherData objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(ExtendedWeatherData.create());
