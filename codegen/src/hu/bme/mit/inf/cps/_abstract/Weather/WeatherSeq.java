...
/**
* A sequence of Weather instances.
*/
public final class WeatherSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public WeatherSeq (int initialMaximum) {
        super(Weather.class, initialMaximum);
    }

    public WeatherSeq (Collection elements) {
        super(Weather.class, elements);
    }

    public  Weather get(int index) { 
        return (Weather) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, Weather.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' Weather objects (beyond the original size of this object)
