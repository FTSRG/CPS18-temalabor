...
/**
* A sequence of TemperatureForecast instances.
*/
public final class TemperatureForecastSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public TemperatureForecastSeq() {
        super(TemperatureForecast.class);
    }

    public TemperatureForecastSeq (int initialMaximum) {
        super(TemperatureForecast.class, initialMaximum);
    }

    public TemperatureForecastSeq (Collection elements) {
        super(TemperatureForecast.class, elements);
    }

    public  TemperatureForecast get(int index) { 
        return (TemperatureForecast) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, TemperatureForecast.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' TemperatureForecast objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(TemperatureForecast.create());
